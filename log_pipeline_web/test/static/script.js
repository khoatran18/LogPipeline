...
document.getElementById("vector-form").onsubmit = function(e) {
  ...
  const boxHtml = `
    ...
    <button onclick="startVector(${currentEditId ?? vectorId}, '${currentType}')" class="status_button start-btn">Start</button>
    <button onclick="restartVector(${currentEditId ?? vectorId}, '${currentType}')" class="status_button restart-btn">Restart</button>
    <button onclick="stopVector(${currentEditId ?? vectorId}, '${currentType}')" class="status_button stop-btn">Stop</button>
    <button onclick="updateVector(${currentEditId ?? vectorId}, '${currentType}')" class="status_button update-btn">Update</button>
    <button onclick="deleteVector(${currentEditId ?? vectorId}, '${currentType}')" class="status_button delete-btn">Delete</button>
  `;
...
function startVector(id, type) {
  ...
  vectors[id].element.style.backgroundColor = "#d4edda";

}

function restartVector(id, type) {
  sendToServer("restart", id, type);
  const element = vectors[id].element;
  element.style.backgroundColor = "#fff3cd"; // vàng
  setTimeout(() => {
    element.style.backgroundColor = "#d4edda"; // chuyển xanh lá
  }, 2000);
}

function stopVector(id, type) {  status_buttons = document.querySelectorAll(".status_button")
  sendToServer("stop", id, type);
  vectors[id].element.style.backgroundColor = "yellow";
}

function updateVector(id, type) {
  showForm(type, id);
  vectors[id].element.style.backgroundColor = "orange";
}

function deleteVector(id, type) {
  sendToServer("delete", id, type);
  const box = vectors[id].element;
  box.remove();
  delete vectors[id];
}
